...
---------------
    ...
    Uses the 1D_{v^2} method: 
       -- solves for one independent variable (v^2) via a 1D
          Newton-Raphson method using the known analytic solution for W
          per value of v^2;
       -- CANNOT be used for arbitrary equations of state, only a Gamma-law
           EOS is compatible;  if you wish to use another EOS, please 
           use utoprim_2d.c, or utoprim_1d.c;
       -- This method is different from utoprim_1dvsq2.c in that it uses
          the closed-form solution of W(v^2) rather than a numerically-calculated
          value via Newton-Raphson;
...
FTYPE Bsq,QdotBsq,Qtsq,Qdotn,D,Wc;
FTYPE Ec, Dc, Ssq, igam1;
...
/*********************************************************************************
   func_1d_dbl(): 
        ...
        -- figures out the right closed form of the solution to use, so that it 
           uses the simplest form and to reduce the error that may arise from 
           non-zero cancelations of the imaginary terms (since answer is always 
           real;
 ...
 *********************************************************************************/
static void func_1d_dbl(FTYPE x[], FTYPE dx[], FTYPE resid[], 
			FTYPE jac[][NEWT_DIM], FTYPE *f, FTYPE *df_out, int n)
{
  FTYPE vsq,glf1,cc,wsq,dw, df ;
  FTYPE a2,a1,a0,q,r,th,w1,vp2,vp3,cosa,dvar,dvarcubed, a2p,z1,z2 ;
  FTYPE discrim, df_check, f_check, fm, dfm;
  ...
  z2 = 1. - igam1*(1. - vsq);
  cc = 1./z2 ;     
  z1 = -(Ec - igam1*glf1*Dc - 0.5*Bsq) ; 
  a2p = z1 +  0.5 * Bsq * vsq;
  a2 = a2p*cc;
  a0 = - 0.5 * Ssq * cc;
  dvar = a2 / 3. ;
  dvarcubed = a2 * a2 * a2 / 27. ;


  // Determine which type of solution it is, and calculate W and dW/dvsq 
  //  accordingly:
  if( a0 < 0. ) {
    if( a2 == 0. ) {                       // Case 3
      Wc = pow( -a0, (1./3.) );
    ...
    else if( dvarcubed == -0.25*a0 ) {     // Case 4 -- rare
      Wc = dvar ;
      //      dw = cc * ( 0.5*Bsq - 0.5*Dc*igam1/glf1 - a2 * igam1 ) / 3. ;
      dw = - igam1 * pow( Ssq, (1./3.) ) * pow( z2, (-4./3.) ) / 6.;
    }
    else if( dvarcubed < -0.25*a0  ) {     // Case 2
      discrim = a0 * ( 0.25*a0 + dvarcubed );
      r = - 0.5 * a0 - dvarcubed;
      Wc = -dvar + pow( (r + sqrt(discrim) ), (1./3.))
            + pow( (r - sqrt(discrim) ), (1./3.))  ; 
      dw = ( Wc*Wc*( igam1*(0.5*Dc/glf1 + a2) - 0.5*Bsq ) + a0*igam1 ) 
	   / ( Wc*( 3.*Wc*z2 + 2.*a2p ) );
    }
    else if( dvarcubed > -0.25*a0  ) {     // Case 1
      cosa = -( dvarcubed + 0.5*a0 ) / dvarcubed;
      if(cosa > 1.) cosa = 1. ;
      th = acos(cosa) ;
      Wc = dvar * ( 2.*cos(th/3.) - 1. ) ;
      dw = ( Wc*Wc*( igam1*(0.5*Dc/glf1 + a2) - 0.5*Bsq ) + a0*igam1 ) 
	   / ( Wc*( 3.*Wc*z2 + 2.*a2p ) );
    }
    else {                               // Invalid case 
  ...
  else if( (a0 == 0.) && (a2 < 0.) ) {            // Case 5 -- rare
    Wc = -a2 ;
    dw = -cc * ( 0.5*Bsq - 0.5*Dc*igam1/glf1 - a2 * igam1 ) ;
  }
  else {     // Invalid case
    *f = FAIL_VAL;
    df = FAIL_VAL;
  ...
  // With the best values for W and dW/dvsq, calculate the rest:
  wsq = Wc*Wc ;
  vp2 = Wc + Bsq;
  vp3 = vp2 + Wc;

  *f  = Qtsq  -  vsq * vp2 * vp2  +  vp3 * Ssq / wsq;
  df  = -vp2 * ( vp2 + 2. * dw * ( vsq  +  Ssq / (wsq*Wc) ) ) ;
  dx[0] = -(*f) / (df) ;
 ...
 ********************************************************************** 
 ...
 The following routines specify the equation of state.  
  The above routines DO assume that the equation of state is as 
  stated below.  If one wishes to use this method with a different 
  EOS, please use Utoprim_1dvsq2() instead and change the EOS 
  routines at the bottom of utoprim_1dvsq.c. 
